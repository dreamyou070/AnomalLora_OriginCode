...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.max_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            random_pixel_index = torch.randperm(query.shape[0])
            query_permute = query[random_pixel_index]
            query_loss = torch.nn.functional.mse_loss(query.float(), query_permute.float(), reduction="none")
