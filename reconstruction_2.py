...
def inference(latent,
    ...
    # [2] unet
    attn_score_2 = timewise_attention(latent, 0, 1)
    attn_score_ = timewise_attention(latent, 499, 500)
...
def main(args):
    ...
    for model in models:
        ...
        # [2] text time embedding
        if args.use_text_time_embedding:
            te_base_dir = os.path.join(parent, f'text_time_embedder')
            pretrained_pe_dir = os.path.join(te_base_dir, f'time_embedder_{lora_epoch}.safetensors')
            text_time_embedding_state_dict = load_file(pretrained_pe_dir)
            text_time_embedding.load_state_dict(text_time_embedding_state_dict)
            text_time_embedding.to(accelerator.device, dtype=weight_dtype)
