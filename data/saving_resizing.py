...
def main(args):
    ...
    for cat in cats:
        if cat == args.trg_cat:
            ...
            train_good_cropping_dir = os.path.join(cat_dir, 'train_cropping/good')
            os.makedirs(train_good_cropping_dir, exist_ok=True)

            rgb_dir = os.path.join(train_good_dir, 'rgb')
            rgb_cropping_dir = os.path.join(train_good_cropping_dir, 'rgb')
            os.makedirs(rgb_cropping_dir, exist_ok=True)

            object_mask_dir = os.path.join(train_good_dir, 'object_mask')
            object_mask_cropping_dir = os.path.join(train_good_cropping_dir, 'object_mask')
            os.makedirs(object_mask_cropping_dir, exist_ok=True)

            images = os.listdir(rgb_dir)

            for image in images:

                img_path = os.path.join(rgb_dir, image)
                object_mask_dir = os.path.join(object_mask_dir, image)
                ...
                object_mask_pil.save(os.path.join(object_mask_cropping_dir, image))
                cropped_img.save(os.path.join(rgb_cropping_dir, image))
