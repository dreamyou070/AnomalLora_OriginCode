...
def main(args):
    ...
    for cat in cats:
        ...
        if cat == args.trg_cat:
            ...
            train_dir = os.path.join(cat_dir, 'test')
            train_cropping_dir = os.path.join(cat_dir, 'test_cropping')
            os.makedirs(train_cropping_dir, exist_ok=True)

            defect_folders = os.listdir(train_dir)
            for defect_folder in defect_folders:
                test_defect_folder = os.path.join(train_dir, defect_folder)
                test_defect_cropping_folder = os.path.join(train_cropping_dir, defect_folder)
                os.makedirs(test_defect_cropping_folder, exist_ok=True)

                rgb_folder = os.path.join(test_defect_folder, 'rgb')
                rgb_cropping_folder = os.path.join(test_defect_cropping_folder, 'rgb')
                os.makedirs(rgb_cropping_folder, exist_ok=True)

                object_mask_folder = os.path.join(test_defect_folder, 'object_mask')
                object_mask_cropping_folder = os.path.join(test_defect_cropping_folder, 'object_mask')
