...
class MVTecDRAEMTrainDataset(Dataset):
    ...
    def __init__(self, root_dir,
        ...
        for folder in folders:
            ...
            for image in images:
                self.image_paths.append(os.path.join(rgb_folder, image))
    ...
    def __getitem__(self, idx):
        ...
        if self.reference_check:
            ...
            if len(self.anomaly_source_paths) > 0:
                group_num = len(self.anomaly_source_paths) // len(self.image_paths)
                start_num = idx * group_num
                end_num = (idx + 1) * group_num
                anomal_src_idx = random.randint(start_num, end_num)
