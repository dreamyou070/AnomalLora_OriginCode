...
class AnomalyDetectionStableDiffusionPipeline_inpaint(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def prepare_latents(
        ...
        generator,
        latents=None,
        image=None, # [1,3,512,512]
        timestep=None,
        is_strength_max=True,
        return_noise=False,
        return_image_latents=False,
    ):
        shape = (batch_size, num_channels_latents, height // self.vae_scale_factor, width // self.vae_scale_factor) # [1,4,64,64]
        ...
        if return_image_latents or (latents is None and not is_strength_max):
            image = image.to(device=device, dtype=dtype)                                    # init image
            image_latents = self._encode_vae_image(image=image, generator=generator)        # init image latents
    ...
    def __call__(self,
                 ...
                 image: Union[torch.FloatTensor, PIL.Image.Image] = None,  # initial image -> pillow
        ...
        prompt_embeds = self._encode_prompt(prompt,device,num_images_per_prompt,do_classifier_free_guidance,
            negative_prompt,prompt_embeds=prompt_embeds,negative_prompt_embeds=negative_prompt_embeds, lora_scale=None)
        ...
        """ [1,1,512,512], [1,3,512,512] , [1,3,512,512] """
        ...
        latents_outputs = self.prepare_latents(batch_size * num_images_per_prompt, num_channels_latents,
                                    height, width, prompt_embeds.dtype, device, generator, latents, image=init_image,
        ...
        if num_channels_unet == 9:
            ...
            if num_channels_latents + num_channels_mask + num_channels_masked_image != num_channels_unet:
                raise ValueError(
                    f"Incorrect configuration settings! The config of `pipeline.unet`: {num_channels_unet} expects"
            ...
            raise ValueError(
                f"The unet should have either 4 or 9 input channels." )
